    ...
    public abstract class ModelValidator : IModelValidator
    {
        ...
        protected virtual void EnsureFieldMapping([NotNull] IModel model)
        {
            foreach (var entityType in model.GetEntityTypes())
            {
                ...
                {
                    var message = propertyBase.FindMemberInfo(forConstruction: true, forSet: true) as string;
                    if (message != null)
                    {
                        ShowError(message);
                    }

                    message = propertyBase.FindMemberInfo(forConstruction: false, forSet: true) as string;
                    if (message != null)
                    {
                        ShowError(message);
                    }

                    message = propertyBase.FindMemberInfo(forConstruction: false, forSet: false) as string;
                    if (message != null)
                    {
                        ShowError(message);
                    }
